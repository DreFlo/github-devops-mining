...
def get_transitions(rows : list, pop_tool_sets : dict, filter_tools_sets : set | None = None) -> dict:
    ...
    print(sorted(list(pop_tool_sets.keys())))
...
def get_fig_dict(pop_tool_sets, transitions, tool_set_colors):
    fig_source = []
    fig_target = []
    fig_value = []
    fig_color = []
    ...
    for period in sorted(list(pop_tool_sets.keys())):
        ...
        for (tool_set_string_source, tool_set_string_target) in transitions[(period, period + 1)]:
            ...
            fig_color.append(','.join(tool_set_colors[tool_set_to_index[tool_set_string_target]].split(',')[:-1]) + ',0.7)')
